...
// Load enrolment
        if (ok) {
            ...
            try {
                ...
                coursemembership = coursemembershipdbloader.loadByCourseAndUserId(b2Context.getCourseId(), user.getId());
...
// Perform requested action
        ...
        if (ok) {
            if (value != null) {
                ...
                xml.append("  <result>\n");
                xml.append("    <sourcedid>").append(Utils.htmlSpecialChars(sourcedid)).append("</sourcedid>\n");
                if (tool.getDoSendUserSourcedid()) {
                    xml.append("    <personsourcedid>").append(Utils.htmlSpecialChars(user.getBatchUid())).append("</personsourcedid>\n");
                }
                if (date.length() > 0) {
                    xml.append("    <date>").append(Utils.htmlSpecialChars(date)).append("</date>\n");
                }
                xml.append("    <resultscore>\n");
                xml.append("      <resultvaluesourcedid>").append(Utils.htmlSpecialChars(type)).append("</resultvaluesourcedid>\n");
                xml.append("      <textstring>").append(Utils.htmlSpecialChars(value)).append("</textstring>\n");
                xml.append("    </resultscore>\n");
                xml.append("  </result>\n");
