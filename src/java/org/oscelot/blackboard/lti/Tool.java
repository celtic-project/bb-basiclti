...
// Format types of text box values
    ...
    private String getToolSetting(boolean global, boolean anonymous, String name, String defaultValue) {
        ...
        } else if (global && (defaultValue == null) || !defaultValue.equals(Constants.DATA_NOTUSED)) {
            value = this.b2Context.getSetting(false, anonymous, setting,
                    this.b2Context.getSetting(true, anonymous, setting, defaultValue));
        } else if (global) {
            value = this.b2Context.getSetting(true, anonymous, setting, defaultValue);
        } else {
    ...
    public String getLaunchSignatureMethod() {
        ...
        String defaultSignatureMethod = "";
        if (this.domain != null) {
            defaultSignatureMethod = this.domain.getSignatureMethod();
        }
        if ((defaultSignatureMethod == null) || (defaultSignatureMethod.length() <= 0)) {
            defaultSignatureMethod = Constants.DATA_SIGNATURE_METHOD_SHA1;
        }
        String signatureMethod = "";
        if (this.byUrl) {
            signatureMethod = this.getToolSetting(false, true, Constants.TOOL_SIGNATURE_METHOD, signatureMethod);
        } else {
    ...
    public String getOpenIn() {
        ...
        boolean allowRender = this.b2Context.getSetting(Constants.TOOL_RENDER, Constants.DATA_FALSE).equals(Constants.DATA_TRUE);
    ...
    public String getWindowName() {
        ...
        String windowName = Constants.DATA_SELF_WINDOW_NAME;
            ...
            if (splashText.startsWith("http://") || splashText.startsWith("https://")) {
                ...
                try {
                    ...
                    if (resp == 200) {
                        ...
                        int pos = splashText.toLowerCase(Locale.ENGLISH).indexOf("<body>");
                        if (pos >= 0) {
                            splashText = splashText.substring(pos + 6);
                        }
                        pos = splashText.toLowerCase(Locale.ENGLISH).indexOf("</body>");
                        if (pos >= 0) {
    ...
    public String getSendExtensions() {
        ...
        if (this.b2Context.getSetting(Constants.TOOL_EXT_OUTCOMES, Constants.DATA_FALSE).equals(Constants.DATA_FALSE)) {
            ...
            setting = this.b2Context.getResourceString("service.outcomes").toLowerCase(Locale.ENGLISH);
        }
        ...
        if (this.b2Context.getSetting(Constants.TOOL_EXT_MEMBERSHIPS, Constants.DATA_FALSE).equals(Constants.DATA_FALSE)) {
            ...
            setting = this.b2Context.getResourceString("service.memberships").toLowerCase(Locale.ENGLISH);
        }
        ...
        if (this.b2Context.getSetting(Constants.TOOL_EXT_SETTING, Constants.DATA_FALSE).equals(Constants.DATA_FALSE)) {
            ...
            setting = this.b2Context.getResourceString("service.setting").toLowerCase(Locale.ENGLISH);
        }
        ...
        if (this.isSystemTool) {
            ...
            for (Iterator<Service> iter = serviceList.getList().iterator(); iter.hasNext();) {
                ...
                if (service.getIsEnabled().equals(Constants.DATA_TRUE)
                        && !ServiceList.isStandardService(service.getClassName())
    ...
    public boolean getDoSendMembershipsService() {
        ...
        if (send) {
            String getM = this.getMembershipsService();
            String getSM = this.getSendMembershipsService();
            send = getM.equals(Constants.DATA_MANDATORY);
            if (!send && !getM.equals(Constants.DATA_NOTUSED)) {
                send = getSM.equals(Constants.DATA_TRUE);
            }
        }
    ...
    public boolean getHasNodeSettings() {

        return (this.b2Context.getSetting(true, true, Constants.TOOL_PARAMETER_PREFIX + "." + this.id, null, this.b2Context.getNode()) != null);
