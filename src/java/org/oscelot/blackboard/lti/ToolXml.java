...
public class ToolXml extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if (!b2Context.getIsRootNode()) {
            subtitle = " [Node: " + b2Context.getNode().getName() + "]";
        }
        ...
        xml.append("  <description>").append(Utils.xmlEncode(description)).append(subtitle).append("</description>\n");
            ...
            for (int i = 0; i < items.length; i++) {
                ...
                if (item.length > 0) {
                    ...
                    if (paramName.length() > 0) {
                        paramName = paramName.toLowerCase(Locale.ENGLISH).replaceAll("[^a-z0-9]", "_");
                        if (item.length > 1) {
