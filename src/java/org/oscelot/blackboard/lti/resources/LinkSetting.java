...
public class LinkSetting extends Resource {
    ...
    public void execute(B2Context b2Context, Response response) {
        ...
        if (ok) {
            ...
            if (b2Context.getRequest().getMethod().equals("GET")) {
                ...
                settingValues = Setting.settingsToJson(contextSettings, simpleFormat, "ToolProxyBinding", contextSetting);
                ...
                json.append(settingValues);
                settingValues = Setting.settingsToJson(linkSettings, simpleFormat, "LtiLink", this);
                if (settingValues.length() > 0) {
                    if (!isFirst) {
                        json.append(",\n");
                    }
    ...
    public String parseValue(String value) {
        ...
        if (b2Context.hasCourseContext() && b2Context.hasContentContext()) {
            String url = this.getEndpoint();
            String contentId = b2Context.getContentIdAsString();
            if (!b2Context.getContent().getContentHandler().equals(Utils.getResourceHandle(b2Context, null))) {
    ...
    private boolean setContext(B2Context b2Context, String contentId) {
        ...
        boolean ok = true;
        if (b2Context.setContentId(contentId)) {
            Content content = b2Context.getContent();
            b2Context.setCourseId(content.getCourseId());
        } else {
            ok = false;
        }
