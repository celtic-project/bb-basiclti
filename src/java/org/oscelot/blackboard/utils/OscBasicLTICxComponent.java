...
public class OscBasicLTICxComponent implements CxComponent {
    ...
    public void doCopy(CopyControl copyControl) {
        ...
        B2Context courseContext = new B2Context();
        courseContext.setCourseId(copyControl.getDestinationCourseId());
        ...
        if (!settings.isEmpty()) {
            ...
            try {
                ...
                ltiUrl = ltiUrl.substring(0, ltiUrl.length() - 1);
            ...
            copyControl.getLogger().logInfo("No settings to process");
        }
        B2Context.processCourseCopy(copyControl);
        boolean doSave;
    ...
    public void doExport(ExportControl exportControl) {
        ...
        B2Context courseContext = new B2Context();
        courseContext.setCourseId(exportControl.getSourceCourseId());
        courseContext.setIgnoreContentContext(true);
        Map<String, String> settings = courseContext.getSettings(false, true);
        if (!settings.isEmpty()) {
                ...
                for (Iterator<Content> iter = items.iterator(); iter.hasNext();) {
                    ...
                    for (Iterator<Content> iter2 = children.iterator(); iter2.hasNext();) {
                        ...
                        if ((destId != null) && destId.isSet()) {
                            ...
                            if (isLTI) {
                                props.setProperty("x_" + destId.toExternalString(), child.getId().toExternalString());
                            }
                        }
                    }
                }
            ...
            props.putAll(B2Context.processCourseExport(exportControl));
    ...
    public void doImport(GenericPackageEntry packageEntry, ImportControl importControl) {
        ...
        B2Context courseContext = new B2Context();
        courseContext.setCourseId(importControl.getDestinationCourseId());
        courseContext.setIgnoreContentContext(true);
        Map<String, String> settings = courseContext.getSettings(false, true);
        ...
        B2Context.processCourseImport(importControl, props);
