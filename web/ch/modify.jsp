...
<bbNG:learningSystemPage title="${bundle['page.content.tool.title']}" entitlement="course.content.MODIFY">
  <%
    ...
    if (request.getMethod().equalsIgnoreCase("POST")) {
      ...
      if (b2Context.getRequestParameter("duedate_checkbox", "").length() > 0) {
        b2Context.setSetting(false, true, toolSettingPrefix + Constants.TOOL_EXT_OUTCOMES_DUEDATE,
           b2Context.getRequestParameter("duedate_datetime", ""));
      } else {
        b2Context.setSetting(false, true, toolSettingPrefix + Constants.TOOL_EXT_OUTCOMES_DUEDATE, "");
      }
      if (!membershipsDisabled) {
      ...
      Calendar dueDate = Utils.getDateFromPicker(
          b2Context.getRequestParameter("duedate_checkbox", ""),
          b2Context.getRequestParameter("duedate_datetime", ""));
      if (dueDate == null) {
        dueDate = Calendar.getInstance();
        dueDate.setTimeInMillis(0L);
      }
      Utils.checkColumn(b2Context, null, content.getTitle(), format, points, scorable, visible, hasOutcomes && addColumn, dueDate);
    ...
    String hasDueDate = "false";
    if (b2Context.getSetting(false, true, toolSettingPrefix + Constants.TOOL_EXT_OUTCOMES_DUEDATE, "").length() > 0) {
      dueDate = Utils.getDateFromPicker("1", b2Context.getSetting(false, true, toolSettingPrefix + Constants.TOOL_EXT_OUTCOMES_DUEDATE, ""));
    }
    Calendar due = Calendar.getInstance();
    due.setTimeInMillis(0L);
    Lineitem lineitem = Utils.checkColumn(b2Context, null, content.getTitle(), null, null, false, false, false, due);
    boolean hasLineItem = lineitem != null;
    if (hasLineItem) {
      OutcomeDefinition def = lineitem.getOutcomeDefinition();
      if (def != null) {
        scorable = def.isScorable();
        visible = def.isVisible();
        params.put(Constants.TOOL_EXT_OUTCOMES_POINTS, Utils.floatToString(def.getPossible()));
        dueDate = def.getDueDate();
      }
    }
    ...
    pageContext.setAttribute("hasDueDate", hasDueDate);
    ...
    <bbNG:dataCollection markUnsavedChanges="true" showSubmitButtons="true">
      <bbNG:step hideNumber="false" title="${bundle['page.content.modify.step1.title']}" instructions="${bundle['page.content.modify.step1.instructions']}">
        ...
        <bbNG:dataElement isRequired="false" label="${bundle['page.content.modify.step1.description.label']}">
          <bbNG:textbox name="${params.descriptionname}" format="${params.descriptiontype}" text="${params.descriptiontext}" helpText="${bundle['page.content.modify.step1.description.description']}" rows="5" isContentLinking="true" />
        </bbNG:dataElement>
        <%
          }
        %>
      ...
      <bbNG:step hideNumber="false" title="${bundle['page.system.tool.step3.title']}" instructions="${bundle['page.system.tool.step3.instructions']}">
        <%
            if (outcomesEnabled) {
              if (hasLineItem) {
                String gbUrl = "/webapps/gradebook/do/instructor/addModifyItemDefinition?actionType=modify&course_id=" + courseId + "&id=" + Utils.getId(lineitem.getId());
                pageContext.setAttribute("gbLink", String.format(b2Context.getResourceString("page.content.modify.step4.lineitem.label"), gbUrl));
        %>
        <bbNG:dataElement isRequired="true" label="${bundle['page.system.tool.step3.outcomes.label']}">
          ...
          <bbNG:elementInstructions text="${gbLink}" />
        </bbNG:dataElement>
        <bbNG:dataElement isSubElement="true" subElementType="INDENTED_NESTED_LIST">
          <bbNG:dataElement isRequired="true" label="${bundle['page.system.tool.step3.outcomes_points.label']}">
            <bbNG:textElement displayOnly="true" type="unsigned_integer" name="<%=Constants.TOOL_EXT_OUTCOMES_POINTS%>" value="<%=params.get(Constants.TOOL_EXT_OUTCOMES_POINTS)%>" title="${bundle['page.system.tool.step3.outcomes_points.label']}" size="5" maxLength="3" helpText="${bundle['page.system.tool.step3.outcomes_points.instructions']}" />
          </bbNG:dataElement>
          <bbNG:dataElement isRequired="true" label="${bundle['page.system.tool.step3.outcomes_scorable.label']}">
            <%
              if (scorable) {
            %>
            <bbNG:radioElement displayOnly="true" optionLabel="${bundle['option.true']}" isSelected="<%=scorable%>" name="<%=Constants.TOOL_EXT_OUTCOMES_SCORABLE%>" value="true" />
            <%
              } else {
            %>
            <bbNG:radioElement displayOnly="true" optionLabel="${bundle['option.false']}" isSelected="<%=!scorable%>" name="<%=Constants.TOOL_EXT_OUTCOMES_SCORABLE%>" value="false" />
            <%
              }
            %>
          </bbNG:dataElement>
          <bbNG:dataElement isRequired="true" label="${bundle['page.system.tool.step3.outcomes_visible.label']}">
            <%
              if (visible) {
            %>
            <bbNG:radioElement displayOnly="true" optionLabel="${bundle['option.true']}" isSelected="<%=visible%>" name="<%=Constants.TOOL_EXT_OUTCOMES_VISIBLE%>" value="true" />
            <%
              } else {
            %>
            <bbNG:radioElement displayOnly="true" optionLabel="${bundle['option.false']}" isSelected="<%=!visible%>" name="<%=Constants.TOOL_EXT_OUTCOMES_VISIBLE%>" value="false" />
            <%
              }
            %>
          </bbNG:dataElement>
          <bbNG:dataElement isRequired="true" label="${bundle['page.content.modify.step4.duedate.label']}">
            <bbNG:checkboxElement isDisabled="true" isSelected="${hasDueDate}" name="duedate_checkbox" id="duedate_checkbox" value="1" />
            <bbNG:datePicker displayOnly="true" dateTimeValue="${dueDate}" checkBoxFieldId="duedate_checkbox" baseFieldName="duedate" showDate="true" showTime="true" />
          </bbNG:dataElement>
        </bbNG:dataElement>
        <%
              } else {
        ...
        <bbNG:dataElement isSubElement="true" subElementType="INDENTED_NESTED_LIST">
          ...
          <bbNG:dataElement isRequired="true" label="${bundle['page.content.modify.step4.duedate.label']}">
            <bbNG:checkboxElement isSelected="${hasDueDate}" name="duedate_checkbox" id="duedate_checkbox" value="1" />
            <bbNG:datePicker dateTimeValue="${dueDate}" checkBoxFieldId="duedate_checkbox" baseFieldName="duedate" showDate="true" showTime="true" />
          </bbNG:dataElement>
        </bbNG:dataElement>
        <%
              }
