...
<%
    ...
    if ((tool.getName().length() <= 0) && (tool.getLaunchUrl().length() <= 0)) {
        ...
        if (!redirect && (tool.getSplash().equals(Constants.DATA_TRUE) || tool.getUserHasChoice())) {
            if ((moduleId != null) || tool.getOpenIn().equals(Constants.DATA_POPUP) || tool.getOpenIn().equals(Constants.DATA_OVERLAY)) {
                if (!B2Context.getIsVersion(9, 1, 201404)) {
                    ...
                    response.sendRedirect("splash.jsp?" + Utils.getQuery(request));
                }
                return;
            }
...
<%
            ...
            if (b2Context.getSetting(false, true, settingPrefix + Constants.TOOL_EXT_UUID, "").length() <= 0) {
                b2Context.setSetting(false, true, settingPrefix + Constants.TOOL_EXT_UUID, UUID.randomUUID().toString());
            ...
            if (b2Context.getRequestParameter("embed", Constants.DATA_FALSE).equals(Constants.DATA_TRUE)) {
                ...
                if (!tool.getOpenIn().equals(Constants.DATA_POPUP) && !tool.getOpenIn().equals(Constants.DATA_OVERLAY)
                    ...
                    if (tool.getOpenIn().equals(Constants.DATA_FRAME_NO_BREADCRUMBS)) {
                        url = "frame2";
                    } else if (!redirect) {
                        if (courseId.length() <= 0) {
                            url = "iframe2";
