...
<bbNG:genericPage title="${title}" bodyClass="popup" onLoad="osc_doOnLoad()" entitlement="system.generic.VIEW">
  ...
  <bbNG:jsBlock>
    <script language="javascript" type="text/javascript">
      ...
      %>
            window.location.href = '<%=contentItemUrl%>&<%=Constants.LINK_TEXT%>=' + encodeURIComponent(osc_el_text.value) + '&<%=Constants.LINK_TITLE%>=' + encodeURIComponent(osc_el_title.value);
      <%
          }
      %>
          }
      ...
      <%
          if (contentItemUrl.length() > 0) {
      %>
          window.location.href = '<%=contentItemUrl%>&<%=Constants.LINK_TITLE%>=' + encodeURIComponent(osc_el_title.value)
      <%
          }
      %>
        }
